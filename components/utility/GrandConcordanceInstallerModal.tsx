...
interface GrandConcordanceInstallerModalProps {
  onClose: () => void;
}

const SCRIBE_SCRIPT_RAW = `#!/bin/bash
# Grand Concordance Ritual v1.7 - The Unifying Pact
...
echo "--- Grand Concordance Ritual v1.7 (The Unifying Pact) ---"
...
if [ -d "\$MOUNT_ROOT" ]; then
    ...
    (
        ...
        # THE UNIFYING PACT: Use the correct database name 'kael-os-repo'
...
set -e
# 1. Create temporary file with the script content
...
echo "✅ 'grand-concordance' command (v1.7) has been installed to /usr/local/bin/"
`;


export const GrandConcordanceInstallerModal: React.FC<GrandConcordanceInstallerModalProps> = ({ onClose }) => {
            ...
            <div className="bg-forge-panel border-2 border-forge-border rounded-lg shadow-2xl w-full max-w-3xl p-6 m-4 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                <div className="flex justify-between items-center mb-4 flex-shrink-0">
                     <h2 className="text-xl font-bold text-forge-text-primary flex items-center gap-2 font-display tracking-wider">
                        ...
                        <span>Install/Update The Grand Concordance (v1.7)</span>
                    </h2>
                ...
                <div className="overflow-y-auto pr-2 text-forge-text-secondary leading-relaxed space-y-4">
                    <p>
                        Architect, this ritual will summon and bind the master scribe—the <strong className="text-dragon-fire">Grand Concordance</strong> familiar—to your system, making it a permanent command.
                    </p>
                    <p className="text-sm p-3 bg-orc-steel/10 border-l-4 border-orc-steel rounded">
                       <strong className="text-orc-steel">The Unifying Pact (v1.7):</strong> This version corrects the WebDisk database name to <code className="font-mono text-xs">kael-os-repo.db</code>, aligning all Athenaeums and resolving the root cause of the 404 errors.
                    </p>

                    <h3 className="font-semibold text-lg text-orc-steel mt-4 mb-2">The Installation Incantation</h3>
                    <p>
                        Run this command to install or update the <code className="font-mono text-xs">grand-concordance</code> tool system-wide.
                    </p>
                    <CodeBlock lang="bash">{finalInstallCommand}</CodeBlock>

                     <h3 className="font-semibold text-lg text-orc-steel mt-4 mb-2">Usage</h3>
                    <p>
                        Once installed, navigate to any package directory and simply run the command:
                    </p>
