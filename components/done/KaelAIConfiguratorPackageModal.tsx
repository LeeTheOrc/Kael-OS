...
const CONFIGURATOR_SCRIPT_HEAD = `#!/bin/bash
...
const FORGE_SCRIPT_HEAD = `#!/bin/bash
set -euo pipefail

# --- CONFIGURATION ---
PKG_DIR="$HOME/forge/packages/kael-ai-configurator"
PKG_NAME="kael-ai-configurator"

# --- FILE CONTENTS (as bash variables) ---
CONFIGURATOR_SCRIPT_CONTENT=$(cat <<'EOF'
`;

const FORGE_SCRIPT_MID1 = `
EOF
)
...
# CLEANUP: Remove old packages to prevent repo-add warnings
...
echo "\$CONFIGURATOR_SCRIPT_CONTENT" > kael-ai-configurator.sh
echo "\$DESKTOP_FILE_CONTENT" > kael-ai-configurator.desktop
...
export const KaelAIConfiguratorPackageModal: React.FC<KaelAIConfiguratorPackageModalProps> = ({ onClose }) => {
    // Construct script via concatenation to avoid template literal mishaps
    const finalForgeScript = 
        FORGE_SCRIPT_HEAD + 
        CONFIGURATOR_SCRIPT_HEAD + 
        FORGE_SCRIPT_MID1 + 
        ...
        <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center animate-fade-in-fast" onClick={onClose}>
            <div className="bg-forge-panel border-2 border-forge-border rounded-lg shadow-2xl w-full max-w-3xl p-6 m-4 flex flex-col max-h-[90vh]" onClick={e => e.stopPropagation()}>
                ...
                <div className="overflow-y-auto pr-2 text-forge-text-secondary leading-relaxed space-y-4">
                    ...
                    <p>
                        If you need to re-forge this artifact, run the command below.
                    </p>
